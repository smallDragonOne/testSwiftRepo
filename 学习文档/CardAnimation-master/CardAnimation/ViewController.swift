...
class ViewController: UIViewController {
    ...
    func createImageViews(count: Int) {
        for index in 0..<count {
            ...
            ///imageView.image = UIImage(contentsOfFile: NSString(format: "Taylor Swift %05d", locale: NSBundle.mainBundle().resourcePath, arguments: [index % 5]))
            
            //let filePath:String = NSBundle.mainBundle().pathForResource("Taylor Swift %05d", ofType: "JPG")!
...
//        imageView.layer.transform = transform
        ...
        imageView.layer.transform = CATransform3DTranslate(imageView.layer.transform, 0, -8 * CGFloat(index), 0)
        ...
        if pan.state == .Changed {
            if abs(trans) > self.maxLength {
                ...
                for index in 0..<self.imageViews.count {
                    ...
                    /// 旋转视图
                    print((1 - (CGFloat(index) / CGFloat(self.imageViews.count))) * (delta / self.maxLength) * (15.0 / 180) * CGFloat(M_PI))
    ...
    func endAnimation() {
        for index in 0..<self.imageViews.count {
            ...
            UIView.animateWithDuration(0.5, delay: NSTimeInterval(index) * 0.1, usingSpringWithDamping: 0.5, initialSpringVelocity: 0, options: UIViewAnimationOptions.CurveLinear, animations: { () -> Void in
                    ...
                    imageView.layer.setValue(-8.0 * CGFloat(index), forKeyPath: "transform.translation.y")
